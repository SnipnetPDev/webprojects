...
return '<span class="badge text-success" style="border:1px solid green; background:#fff;">'.$extra.''.$yfinal.' '.$mfinal.' '.$dfinal.'</span>';
}else{
 ...
 if ( 0 < $_FILES["$perimeter"]['error'] ) {
	  return json_encode([
		...
		if (empty($errors)) {
            ...
            $base64 = '/' . $file_ext . ';base64,' . base64_encode($data);
			return json_encode([
                "status" => 'ok',
                "message" => $base64,
...
function mkBase64File($base64file){ 
    $mimeType = '';
    $str = ltrim($base64file, '/'); 
    $ext = explode(';', $str);
	$extension = $ext[0];
	if($extension == 'pdf'){
	...
	$compile = "data:$mimeType;".$ext[1]."";
	if($mimeType == ''){
	return json_encode([
                "status" => 'bad',
                "message" => 'Invalid perimeter supplied'
            ]);  
	}else{
	return json_encode([
                "status" => 'ok',
