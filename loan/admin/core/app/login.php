...
if($CPreport !== ''){
	...
	$sql = "SELECT * FROM accounts WHERE mobile = '$mobile' and usrRole = 'admin'";
	...
	echo '<h2 class="text-7 text-center">Verify '.$mobile.'</h2>
...
if($custom->sendSMS($mobile, $smsMSG, KEY0, KEY1, KEY2) == 'sent'){
	if($mysqli->insertInto('safe_token',array('tres' => $mobile, 'tkey' => $_SESSION['code'], 'tip' => $guest_ip, 'ttime' => $app->get_date_time('dateTime'))) == 'done'){
		...
		<input type="hidden" class="form-control" id="mode" value="login">
		  ...
		  <div class="form-group">
                            ...
                            <input type="number" id="Vcode" name="Vcode" onkeyup="verify(this.value);" pattern="\d{6}" maxlength="6" class="form-control" placeholder="******" required="" autofocus="">
                 </div>
			<div class="form-group">
			<span id="Vreport"></span>
			 </div>
	...
	echo '<center><font style="background:red;color:#fff;padding:5px;">Something went wrong, please try again</font></center>';
				...
				echo "<center><font style='color:red;'>Access denied, Admin privilages only.</font></center>";
				exit;
			}
...
$resID = $resID;
	...
	$logID = $app->get_rand_numbers(12);
			...
			if($Row_count > 0) {
				...
				echo "<center><font style='color:red;'>You currently do not have an account with us, kindly complete our online <a href='apply'>application form</a> to get your instant ".APP_NAME." account.</font></center>";
				exit;
				}
}else{
				echo 1;
...
if($app->checkParameter($_POST["mobileOtp"], 'number') == 'pass'){
	$mobileOtp = $_POST['mobileOtp'];
}else{
	$CPreport = 'Phone number '.$app->checkParameter($_POST["mobileOtp"], 'number');
}
...
$_SESSION['code'] = $app->get_rand_numbers(6);
$smsMSG = APP_NAME.' verification code '.$_SESSION['code'];
if($custom->sendSMS($mobileOtp, $smsMSG, KEY0, KEY1, KEY2) == 'sent'){
	if($mysqli->insertInto('safe_token',array('tres' => $mobileOtp, 'tkey' => $_SESSION['code'], 'tip' => $app->getWhois(), 'ttime' => $app->get_date_time('dateTime'))) == 'done'){
						echo '<h2 class="text-7 text-center">Verify '.$mobileOtp.'</h2>
        <p class="text-3 text-center mb-25">Enter the code you have received from '.APP_NAME.'</p> <br/>
          <div class="vertical-input-group">
		  <div class="form-group">
                            <label for="inputEmail" class="sr-only">OTP</label>
                            <input type="number" id="OtpCode" onkeyup="verifyOtp(this.value);" pattern="\d{6}" maxlength="6" class="form-control" placeholder="Enter the code you have received" required="" autofocus="">
                 </div>
			<div class="form-group">
			<span id="OtpReport"></span>
			 </div>
          </div>
		';
	}else{
...
if (isset($_POST['OtpCode']) && isset($_POST['OtpresID'])) {
if($app->checkParameter($_POST["OtpCode"], 'string') == 'pass'){
	$OtpCode = $_POST['OtpCode'];
}else{
	$CPreport = 'Verification code '.$app->checkParameter($_POST["OtpCode"], 'string');
}
if($app->checkParameter($_POST["OtpresID"], 'string') == 'pass'){
	$OtpresID = $_POST['OtpresID'];
}else{
	$CPreport = 'Phone number '.$app->checkParameter($_POST["OtpresID"], 'string');
}
if($CPreport !== ''){
	echo "<center><font style='background:red;color:#fff;padding:5px;'>$CPreport</font></center>";
	exit;
}
		    $sql = "SELECT * FROM safe_token WHERE tkey = '$OtpCode' AND tres = '$OtpresID'";
			$data = $dbObj->query_execute($sql);
			$Row_count = $dbObj->query_rowCount($data);
			if($Row_count > 0) {
							$del_sql = "DELETE FROM safe_token WHERE tkey = '$OtpCode' AND tres = '$OtpresID'";
			                $dbObj->query_execute($del_sql);
							echo 0;
...
if($app->checkParameter($_POST["userToreset"], 'string') == 'pass'){
	$userToreset = $_POST['userToreset'];
}else{
	$CPreport = 'UserId '.$app->checkParameter($_POST["userToreset"], 'string');
}
if($app->checkParameter($_POST["userPin"], 'string') == 'pass'){
	$userPin = $_POST['userPin'];
}else{
	$CPreport = 'newpin '.$app->checkParameter($_POST["userPin"], 'string');
}
if($app->checkParameter($_POST["userCnpin"], 'string') == 'pass'){
	$userCnpin = $_POST['userCnpin'];
}else{
	$CPreport = 'newpin2 '.$app->checkParameter($_POST["userCnpin"], 'string');
}
if($CPreport !== ''){
	echo "<center><font style='background:red;color:#fff;padding:5px;'>$CPreport</font></center>";
	exit;
}
$upd_sql = "UPDATE accounts SET pass = '$userCnpin' WHERE mobile = '$userToreset'";
			        if($dbObj->query_execute($upd_sql)) {
						echo '<center><h3>Pin reset complete!</h3><br/><p>You can now login using your new Pin</p></center>';
					}else{
						echo 1;
					}
}
